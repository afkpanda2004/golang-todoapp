...
import (
	...
	core_http_types "github.com/afkpanda2004/golang-todoapp/internal/core/transport/http/types"
)

type PatchUserRequest struct {
	FullName    core_http_types.Nullable[string] `json:"full_name"     swaggertype:"string" example:"Leonid Belyaev"`
	PhoneNumber core_http_types.Nullable[string] `json:"phone_number"  swaggertype:"string" example:"+79118765657"`
}
...
func (r *PatchUserRequest) Validate() error {
	if r.FullName.Set {
		...
		if fullNameLen < 3 || fullNameLen > 100 {
			return fmt.Errorf("`FullName` must be bettwen 3 and 100 symbols")
		}
	}

	if r.PhoneNumber.Set {
		if r.PhoneNumber.Value == nil {
			phoneNumberLen := len([]rune(*r.FullName.Value))
			if phoneNumberLen < 10 || phoneNumberLen > 15 {
				return fmt.Errorf("`PhoneNumber` must be bettwen 10 and 15 symbols")
			}
...
func (h *UserHTTPHandler) PatchUser(rw http.ResponseWriter, r *http.Request) {
	...
	if err := core_http_request.DecodeAndValidateRequest(r, &request); err != nil {
		responseHandler.ErrorResponse(
			err,
			"failde to decode and validate HTTP request",
		)
